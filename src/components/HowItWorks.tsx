      ...
      <div className="container mx-auto px-4">
        <div className="text-center mb-16">
          ...
          <p className="text-lg text-neutral/70 max-w-2xl mx-auto font-inter">
            To' Barato es fácil de usar y te ayuda a ahorrar dinero en cada
            compra con solo seguir estos simples pasos.
          </p>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8 relative z-10">
            {steps.map((step, index) => (
              <div key={index} className="flex flex-col items-center">
                <div
                  className={`${step.color} w-16 h-16 rounded-full flex items-center justify-center mb-12 shadow-lg`}
                >
                  {step.icon}
                </div>
                <h3 className="text-xl font-semibold mb-2 font-poppins text-center">
                  {step.title}
                </h3>
                <p className="text-neutral/70 text-center font-inter">
                  {step.description}
                </p>
              </div>
            ))}
